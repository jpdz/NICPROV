...
// added
	res = doca_mmap_create(NULL, &state->src_mmap2);
	if (res != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Unable to create source mmap: %s", doca_get_error_string(res));
		return res;
	}
	res = doca_mmap_dev_add(state->src_mmap2, state->dev);
	if (res != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Unable to add device to source mmap: %s", doca_get_error_string(res));
		doca_mmap_destroy(state->src_mmap2);
		state->src_mmap2 = NULL;
		return res;
	}
	//

		res = doca_mmap_create(NULL, &state->src_mmap3);
	if (res != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Unable to create source mmap: %s", doca_get_error_string(res));
		return res;
	}
	res = doca_mmap_dev_add(state->src_mmap3, state->dev);
	if (res != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Unable to add device to source mmap: %s", doca_get_error_string(res));
		doca_mmap_destroy(state->src_mmap3);
		state->src_mmap3 = NULL;
		return res;
	}

	//

		res = doca_mmap_create(NULL, &state->src_mmap4);
	if (res != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Unable to create source mmap: %s", doca_get_error_string(res));
		return res;
	}
	res = doca_mmap_dev_add(state->src_mmap4, state->dev);
	if (res != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Unable to add device to source mmap: %s", doca_get_error_string(res));
		doca_mmap_destroy(state->src_mmap4);
		state->src_mmap4 = NULL;
		return res;
	}

	//

		res = doca_mmap_create(NULL, &state->src_mmap5);
	if (res != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Unable to create source mmap: %s", doca_get_error_string(res));
		return res;
	}
	res = doca_mmap_dev_add(state->src_mmap5, state->dev);
	if (res != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Unable to add device to source mmap: %s", doca_get_error_string(res));
		doca_mmap_destroy(state->src_mmap5);
		state->src_mmap5 = NULL;
		return res;
	}

		res = doca_mmap_create(NULL, &state->src_mmap6);
	if (res != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Unable to create source mmap: %s", doca_get_error_string(res));
		return res;
	}
	res = doca_mmap_dev_add(state->src_mmap6, state->dev);
	if (res != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Unable to add device to source mmap: %s", doca_get_error_string(res));
		doca_mmap_destroy(state->src_mmap6);
		state->src_mmap6 = NULL;
		return res;
	}
//added
...
//added
	if (state->src_mmap2 != NULL) {
		tmp_result = doca_mmap_destroy(state->src_mmap2);
		if (tmp_result != DOCA_SUCCESS) {
			DOCA_ERROR_PROPAGATE(result, tmp_result);
			DOCA_LOG_ERR("Failed to destroy source mmap: %s", doca_get_error_string(tmp_result));
		}
	...
	//
		...
		if (state->src_mmap3 != NULL) {
		tmp_result = doca_mmap_destroy(state->src_mmap3);
		if (tmp_result != DOCA_SUCCESS) {
			DOCA_ERROR_PROPAGATE(result, tmp_result);
			DOCA_LOG_ERR("Failed to destroy source mmap: %s", doca_get_error_string(tmp_result));
		}
	...
	if (state->src_mmap4 != NULL) {
		tmp_result = doca_mmap_destroy(state->src_mmap4);
		if (tmp_result != DOCA_SUCCESS) {
			DOCA_ERROR_PROPAGATE(result, tmp_result);
			DOCA_LOG_ERR("Failed to destroy source mmap: %s", doca_get_error_string(tmp_result));
		}
	...
	if (state->src_mmap5 != NULL) {
		tmp_result = doca_mmap_destroy(state->src_mmap5);
		if (tmp_result != DOCA_SUCCESS) {
			DOCA_ERROR_PROPAGATE(result, tmp_result);
			DOCA_LOG_ERR("Failed to destroy source mmap: %s", doca_get_error_string(tmp_result));
		}
		...
		if (state->src_mmap6 != NULL) {
		tmp_result = doca_mmap_destroy(state->src_mmap6);
		if (tmp_result != DOCA_SUCCESS) {
			DOCA_ERROR_PROPAGATE(result, tmp_result);
			DOCA_LOG_ERR("Failed to destroy source mmap: %s", doca_get_error_string(tmp_result));
		}
