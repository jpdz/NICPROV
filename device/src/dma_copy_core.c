...
static int _parse(FILE *out, struct ppm_evt_hdr *hdr, char *buffer, void *__data)
{
	...
	// we need it
	//printf("%lu %u : %lld %s type is %lld\n", hdr->ts, hdr->tid, info->nparams, info->name, hdr->type);
...
uint32_t parse_main(char *buffer, uint32_t value_previous) {
    char *ptr, *buffer_end;
	...
	// printf("the buffer 31\n");
    // ptr = buffer + 4 + 2 * 30 * 1024 * 1024;
    // buffer_end = ptr + 1024;
	...
	//we need it
	// printf("The previous tail is: %lu\n",value_previous);
	// ptr = buffer + value_previous;
	// hdr = (struct ppm_event_hdr *)ptr;
    // _parse(file, hdr, (char *)(hdr + 1), 0);
	...
	//we need it
	//printf("The current tail is: %lu\n",value);
	double intervel;
	intervel = (float) value*5/2097152;
	printf("%.2f\n",intervel);

	// printf("the buffer 2\n");
	// ptr = buffer + 4 + 2 * 1024 * 1024;
    // buffer_end = ptr + 100;
    // while (ptr < buffer_end) {
    //     hdr = (struct ppm_event_hdr *)ptr;
    //     _parse(file, hdr, (char *)(hdr + 1), 0);
...
static doca_error_t
dpu_submit_dma_job(struct dma_copy_cfg *cfg, struct core_state *core_state, size_t bytes_to_copy, char * buffer,
		   struct doca_buf *local_doca_buf, struct doca_buf *local_doca_buf2, struct doca_buf *remote_doca_buf)
{
	struct doca_event event = {0};
	struct doca_dma_job_memcpy dma_job = {0};
	doca_error_t result;
	...
	clock_t start, end, start_previous;
	start_previous = 0;
	...
	struct doca_buf * local_doca_buf_list[2];
	local_doca_buf_list[0] = local_doca_buf;
	local_doca_buf_list[1] = local_doca_buf2;
	...
	//start = clock();
	struct timeval begin_new, end_new;
	gettimeofday(&begin_new, 0);

	dst_buf = local_doca_buf_list[i];
	result = doca_buf_get_data(src_buf, &data);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to get data address from DOCA buffer: %s", doca_get_error_string(result));
		return result;
	...
	//i ^= 1; 
	//end = clock();
	value_previous = parse_main(buffer, value_previous);
	//printf("time is %lfs --------------------------------------------------------\n", (double)(end - start) / CLOCKS_PER_SEC);

	//printf("it is %lld \n",CLOCKS_PER_SEC);
	gettimeofday(&end_new, 0);
	//printf("%ld\n",((end_new.tv_sec - begin_new.tv_sec) * 1000000 + (end_new.tv_usec - begin_new.tv_usec)) /1000);
	//we need it
	//printf("%ld\n",((end_new.tv_sec - begin_new.tv_sec) * 1000000 + (end_new.tv_usec - begin_new.tv_usec)) );
	...
	doca_buf_reset_data_len(dst_buf);
	// printf("start intervel is %lfs --------------------------------------------------------\n", (double)(start - start_previous) / CLOCKS_PER_SEC);
		   ...
		   struct doca_comm_channel_addr_t **peer_addr)
{
	...
	struct doca_buf *local_doca_buf2;
	...
	//added
	result = doca_buf_inventory_buf_by_addr(core_state->buf_inv, core_state->mmap, buffer + host_dma_offset, host_dma_offset,
						&local_doca_buf2);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Unable to acquire DOCA local buffer: %s", doca_get_error_string(result));
		send_status_msg(ep, peer_addr, STATUS_FAILURE);
	...
	result = dpu_submit_dma_job(dma_cfg, core_state, host_dma_offset, buffer, local_doca_buf, local_doca_buf2,remote_doca_buf);
