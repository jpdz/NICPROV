 ...
 * GLOBALS smartnic
 */
#define DEFAULT_BUFFER_BYTES_DIM 2 * 1024 * 1024;
...
cleanup_ioctl_procinfo:
	...
	//    if (copy_from_user(&addr, (uintptr_t)arg, sizeof(uintptr_t)))
	//     {
	//         return -EFAULT;
	//     }
	{	
		...
		addr = (uintptr_t)arg;
		pr_info("Buffer Address %lx\n", addr);
		// vma = find_vma(current->mm, addr);
		// page = follow_page(vma, addr, FOLL_REMOTE);
		...
		// haddr = __pfn_to_phys(page_to_pfn(page));
		// pr_info("Buffer Address now %lx\n", haddr);
		// kmem = vaddr;
		ring->buffer = vaddr;
		memcpy(ring->buffer, "Hello, sysdig user!", 12);
		// pr_info("")
		goto cleanup_ioctl;
	}
	default:
...
// 			pgprot_val(vma->vm_page_prot) = pgprot_val(PAGE_SHARED) | _PAGE_ENC;
// 			//printk("111");
// 			//pgprot_t prot = pgprot_device(vma->vm_page_prot);
// 			//pgprot_val(vma->vm_page_prot) = pgprot_val(prot) | _PAGE_ENC;
...
// 			goto cleanup_mmap;
// 		}
// 		else if(length == consumer->buffer_bytes_dim)
// 		{
// 			/*
// 			 * When the size requested by the user equals the ring buffer size, we map the full
// 			 * buffer
...
// 			if (flags & VM_WRITE) {
//         		printk(KERN_INFO "Received MAP_PRIVATE flag\n");
//   			}	

// 			printk("dsa");

// 			//ret = remap_vmalloc_range(vma, vmalloc_area_ptr, 0);
// 			// pfn = vmalloc_to_pfn(vmalloc_area_ptr);
// 			ret = remap_pfn_range(vma, vma->vm_start, vmalloc_area_ptr,
...
// 			// 	//printk("property %d", vma->vm_page_prot);
// 			// 	// ret = remap_pfn_range(vma, useraddr, pfn,
// 			// 	// 		      PAGE_SIZE, vma->vm_page_prot);
...
// 			// 	//printk("333");
// 			// 	//pgprot_val(vma->vm_page_prot) = pgprot_val(PAGE_SHARED) | _PAGE_ENC;
// 			// 	pgprot_t prot = prot = pgprot_device(vma->vm_page_prot);
// 			// 	pgprot_val(vma->vm_page_prot) = pgprot_val(prot) | _PAGE_ENC;
...
// convert
void write_u32_to_buffer(__u32 data, uint8_t* buffer) {
    buffer[3] = (data >> 24) & 0xFF;  // 获取低位字节
    buffer[2] = (data >> 16) & 0xFF;
    buffer[1] = (data >> 8) & 0xFF;
    buffer[0] = data & 0xFF;  // 获取高位字节
}
				 ...
				 kmod_prog_codes tp_type)
{
	...
	// if (current->common !=33){
	// 	return res;
	// }

	//if (strcmp(current->comm, "redis-server")){
	//	return res;
  	//}
	...
	//if (cpu == 0){
	//	end = ktime_get_real();
	//	//s64 time_diff = ktime_to_ns(ktime_sub(end, start));
	//	printk("%lld, %lld\n",count, end-start);
	//}
	...
	// 	pr_err("%d",ring_info->tail);
    // }
	 if (unlikely(ring->overflow.filled == 1)) {
        ring_info->tail = ((struct ppm_evt_hdr *)ring->overflow.addr)->len + 4;
        //ring_info->n_evts++;

        memmove(ring->buffer + 4, ring->overflow.addr, ring_info->tail -4);

        ring->overflow.filled = 0;
		// if (cpu == 0){
		// 	hdr = (struct ppm_evt_hdr *)(ring->buffer);
		// 	pr_err("len1 is %lld, time is %llu, event is %d, len2 is %lld",ring_info->tail, hdr->ts,hdr->type,hdr->len);
		// }
    }
...
restart:
    if (freespace < args.arg_data_offset + sizeof(struct ppm_evt_hdr) || restart) {
		...
		force = 1;
		//printk("page size is %lu",PAGE_SIZE);
    } else {
...
// #endif
		...
		default:
			...
			{
				//u32 ttail = ring_info->tail;
				memcpy(ring->buffer, &(ring_info->tail), sizeof(uint32_t));
...
static int init_ring_buffer(struct ppm_ring_buffer_context *ring, unsigned long buffer_bytes_dim)
{
	...
	// ring->buffer = kmalloc(buffer_bytes_dim + 2 * PAGE_SIZE,GFP_KERNEL);
	// if (ring->buffer == NULL) {
